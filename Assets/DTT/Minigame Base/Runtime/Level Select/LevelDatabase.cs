    ...
    public class LevelDatabase : ScriptableObject
    {
        ...
        public void ResetData()
        {
            ...
            if (File.Exists(completePath))
            {
                ...
                Debug.Log("Data reset: JSON file deleted.");
            }
            else
            {
            ...
            // Optionally, reset _data to default values if needed
            _data = new LevelData[_levelCount];
            for (int i = 0; i < _levelCount; i++)
            {
                _data[i] = new LevelData { levelNumber = i + 1, locked = true };
            }
                _data[0] = new LevelData { levelNumber = 0 + 1, locked = false };
        }
    }
}